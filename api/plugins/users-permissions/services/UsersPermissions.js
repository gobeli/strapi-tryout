...
module.exports = {
  ...
  getPlugins: (plugin, lang = 'en') => {
    return new Promise((resolve, reject) => {
      request(
        {
          proxy: 'http://proxy.corproot.net:8079',
  ...
  getActions: (plugins = [], withInfo = true) => {
    ...
    const appControllers = Object.keys(strapi.api || {}).reduce(
      (acc, key) => {
  ...
  updatePermissions: async function(cb) {
    ...
    const appActions = Object.keys(strapi.api || {}).reduce((acc, api) => {
      Object.keys(_.get(strapi.api[api], 'controllers', {})).map(controller => {
    ...
    const pluginsActions = Object.keys(strapi.plugins).reduce((acc, plugin) => {
      Object.keys(strapi.plugins[plugin].controllers).map(controller => {
    ...
    if (!_.isEqual(actions, currentActions) || permissions < 1) {
      ...
      const defaultPolicy = (obj, role) => {
        const isCallback =
          obj.action === 'callback' && obj.controller === 'auth' && obj.type === 'users-permissions' && role.type === 'public';
        const isConnect = obj.action === 'connect' && obj.controller === 'auth' && obj.type === 'users-permissions';
        const isRegister =
          obj.action === 'register' && obj.controller === 'auth' && obj.type === 'users-permissions' && role.type === 'public';
        const isPassword =
          obj.action === 'forgotpassword' && obj.controller === 'auth' && obj.type === 'users-permissions' && role.type === 'public';
        const isNewPassword =
          obj.action === 'changepassword' && obj.controller === 'auth' && obj.type === 'users-permissions' && role.type === 'public';
        ...
        const enabled =
          isCallback || isRegister || role.type === 'root' || isInit || isPassword || isNewPassword || isMe || isReload || isConnect;
      ...
      const toAdd = (permissions < 1 ? currentActions : _.difference(currentActions, actions)).map(splitted);
          ...
          .map(role =>
            Promise.all(
              toAdd
                ...
                .map(action =>
                  strapi.query('permission', 'users-permissions').addPermission(Object.assign(action, { role: role.id || role._id }))
                )
            )
          )
          .concat([Promise.all(toRemove.map(action => strapi.query('permission', 'users-permissions').removePermission(action)))])
      );
    ...
    const value = permissions.reduce(
      (acc, permission) => {
        const index = acc.toKeep.findIndex(
          element =>
            element === `${permission.type}.controllers.${permission.controller}.${permission.action}.${permission.role[primaryKey]}`
        );
